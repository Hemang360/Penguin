...
func (s *StorageService) StoreArtwork(ctx context.Context, data []byte, metadata *DAGMetadata) (string, string, error) {
    ...
    if s.pinata != nil && s.pinata.Enabled() {
        // Prefer Pinata when configured
        filename := fmt.Sprintf("%s.png", metadata.ArtworkID)
        cid, err = s.pinata.UploadBytes(data, filename)
        if err != nil {
    ...
    if len(txHash) > 66 { txHash = txHash[:66] }
...
func (c *IPFSClient) DownloadFile(cid string) ([]byte, error) {
	...
	if !ok {
        ...
        if gateway == "" { gateway = "https://gateway.pinata.cloud/ipfs" }
...
func httpGet(url string) ([]byte, error) {
    ...
    if resp.StatusCode != 200 { return nil, fmt.Errorf("bad status: %s", resp.Status) }
